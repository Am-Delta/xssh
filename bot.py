...
def add_test_user(chat_id):
    for i in range(5):
        try:
            cur.execute("INSERT INTO Tests (ID, 'date') VALUES (?, ?)", (chat_id, str(datetime.now())))
            conn.commit()
            break
        except:
            pass
...
def call_settings(bot, query):
    keyboard = [
        ...
        [InlineKeyboardButton("🆓 اکانت تست ", callback_data='TASET')],
